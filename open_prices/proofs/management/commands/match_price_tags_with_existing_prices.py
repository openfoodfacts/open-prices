...
proof_qs = (
    ...
    .prefetch_related("prices", "price_tags", "price_tags__predictions")
    .with_stats()
    .filter(price_count_annotated__gt=0, price_tag_count_annotated__gt=0)
    .exclude(price_count_annotated=F("price_tag_count_annotated"))
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:  # type: ignore
        ...
        for index, proof in enumerate(proof_qs):
            for price_tag in proof.price_tags.filter(price_id__isnull=True).exclude(
                ...
                for price in proof.prices.all():
                    ...
                    # match product price
                        ...
                        and match_product_price_tag_with_product_price(price_tag, price)
                    ):
